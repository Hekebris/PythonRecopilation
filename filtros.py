...
filtro_gaussiano = np.array([[1,5,10,5,1],
                            [5,15,20,15,5],
                            [10,20,40,20,10],
                            [5,15,20,15,5],
                            [1,5,10,5,1]])

floppa_filtrado_gauss = floppa.copy()
    ...
    for columna in range(2,columnas-2):
        ...
        for filaK in range(fila-2, fila+3):
            ...
            for colK in range(columna-2, columna+3):
                suma = suma + floppa[filaK][colK] * filtro_gaussiano[i][j]
            
                j += 1
            i += 1
        floppa_filtrado_gauss[fila][columna] = suma/290
  
cv.imshow('floppa sin instagram', floppa)
cv.imshow('floppa con instagram', floppa_filtrado_gauss)
cv.waitKey()